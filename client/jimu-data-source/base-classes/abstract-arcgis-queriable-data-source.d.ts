...
export declare abstract class AbstractArcGISQueriableDataSource extends AbstractArcGISQueriableDataSource_base implements ArcGISQueriableDataSource {
    ...
    private createFeatureLayerPromise;
    private lastEditTimeCheckTime;
    private lastEditTimeCheckPromise;
    private readonly cachedArcadeExecutorInfoObj;
    ...
    loadAll(query: ArcGISQueryParams, signal?: AbortSignal, progressCallback?: QueryProgressCallback, options?: QueryOptions): Promise<FeatureDataRecord[]>;
    loadById(id: string | number, refresh?: boolean): Promise<FeatureDataRecord>;
    queryById(id: string | number, fields?: string[]): Promise<FeatureDataRecord>;
    getSourceRecords(): FeatureDataRecord[];
    getRecordById(id: string | number): FeatureDataRecord;
    getRecords(): FeatureDataRecord[];
    getRecordsWithSelection(): FeatureDataRecord[];
    getRecordsByPage(page: number, pageSize: number): FeatureDataRecord[];
    getRecordsByPageWithSelection(page: number, pageSize: number): FeatureDataRecord[];
    ...
    private serializeComplexUniqueIds;
    doQueryById(id: string | number, fields?: string[]): Promise<FeatureDataRecord>;
    ...
    getHistoricMoment(): string | number;
    changeHistoricMoment(historicMoment: string | number): void;
     ...
     * @ignore
     */
    isArcadeField(fieldName: string): boolean;
    /**
     * `usedFieldNames` maybe include Arcade fields, this method will convert them to real field names.
     * For example, the value of `usedFieldNames` is ['state_name', 'exbArcadeField1'].
     * `state_name` is a service field. `exbArcadeField1` is a custom Arcade field that uses the service fields `sub_region` and `state_abbr`.
     * Therefore, the returned result is ['state_name', 'sub_region', 'state_abbr'].
     * @param usedFieldNames
     */
    convertUsedFieldsToSchemaFields(usedFieldNames: string[]): string[];
    /**
     * @ignore
     */
    createArcadeExecutorForArcadeField(arcadeFieldName: string): Promise<__esri.ArcadeExecutor>;
    /**
     * Create and cache arcade executor for custom arcade field or popup expression.
     * The arcade executor will be created only once for each arcade field and cached in main data source.
     * This method never rejects, if there is any error during creating arcade executor, it will be caught and logged, and return null.
     * @param arcadeFieldName
     * @param arcadeScript
     */
    private createCachedArcadeExecutor;
    private rawCreateArcadeExecutor;
}
export {};
