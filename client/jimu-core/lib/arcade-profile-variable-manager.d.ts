...
/**
 * Wraps an async function with debounce behavior. The debounced async function has the same signature (both input and output) with the original async function.
 * All returned Promises resolve (or reject) with the result of that last request.
 *
 * @template T The type of data returned by asyncFunction
 * @param asyncFunction Original async function
 * @param wait Debounce wait time in milliseconds
 * @returns A debounced version of asyncFunction
 */
export declare function createDebouncedAsyncFunction<T>(asyncFunction: (...args: any[]) => Promise<T>, wait: number): (...args: any[]) => Promise<T>;
