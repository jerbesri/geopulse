...
/**
 * The switch account parameters.
 */
interface SwitchAccountParams {
    /** The portal URL to switch account. This is optional. The default is the main portal URL. To switch account for a federated service URL, you need to pass in its federated portal URL. */
    desUrl?: string;
    /** The OAuth2 client id. */
    clientId?: string;
    /** If 'true', will use a popup window to switch account, if 'false', it will redirect to switch account page in the same window. This is optional. The default is true. */
    popup?: boolean;
    /** The url of federated service if switch account to a federated service. Optional. @ignore */
    federatedServiceUrl?: string;
}
/**
 * The sign-out parameters.
 */
interface SignOutParams {
    /** The portal URL to sign-out to. This is optional. The default is the main portal URL. */
    desUrl?: string;
    /** The OAuth2 client id. */
    clientId?: string;
    /** If 'true', it will redirect to sign-out page in the same window, if 'false', will use static sign-out without page redirected or refreshed. This is optional. The default is true. */
    redirectToSignOutPage?: boolean;
    /** The redirected url after sign-out. Optional. The default is the root path, only take effect when 'redirectToSignOutPage' is true */
    redirectUrl?: string;
}
interface HandleAuthErrorParams {
    error: any;
    urlOfAuthError?: string;
    gotoSignInDirectly?: boolean;
    popup?: boolean;
}
...
export declare class SessionManager {
    ...
    private sessions;
    /**
     * The trusted servers list. If a server is in the trusted server list, it cannot be guaranteed that it must be a web-tier server
     */
    private trustedServers;
    private webTierResources;
    private isSigning;
    ...
    /**
     * Init resource sessions from storage, validate the token before init it.
     */
    private initResourceSessionsFromStorage;
    /**
     ...
     * @param session
     */
    readonly isMainSession: (session: ArcGISIdentityManager) => boolean;
    /**
    ...
    /**
     * @ignore
     * Get main user info by main session.
     * @param newMainSession
     */
    getUserInfo(newMainSession?: ArcGISIdentityManager): Promise<IUser>;
    /**
    ...
    /**
     * Return true if the portal url is the same as main portal url.
     * @param portalUrl
     * @ignore
     */
    isMainPortal(portalUrl: string): boolean;
    /**
     * @ignore
    ...
    /**
     * @ignore
     * Return true if the server url is set in trusted server list.
     * @param serverUrl
     */
    isTrustedServer(serverUrl: string): boolean;
    /**
     * return true if the resource url is web-tier resource
     * @param resourceUrl: the resource url is service url or server root url for server resource,
     * and is portal url for portal or item resource.
     * Note: resource url also can be a server root url. (e.g. https://sampleserver6.arcgisonline.com/arcgis)
     * @ignore
     */
    isWebTierResource(resourceUrl: string): boolean;
    /**
     * Add the resource url to web-tier resource list, and sync to other window.
     * @param resourceUrl: the resource url is service url or server root url for server resource,
     * and is portal url for portal or item resource
     * @ignore
     */
    private addToWebTierResourceList;
    /**
     * Get web-tier resources.
     * @ignore
     */
    getWebTierResources(): string[];
    /**
     * Remove the session from session manager and revoke token of session. So, deleting a session means signing out.
     * Note: token of stand-alone server session will be destroyed when syncing to 'arcgis-dependency-extension'
    ...
    /**
     * Clear session from session manager by key, the token is not be revoked
     * @param key
     */
    private clearSessionByKey;
    /**
     * @ignore
     *
     ...
     */
    private setSession;
    private updatingAfterMainSessionChanged;
    ...
    /**
     * update resource auth info (exclude main session) to session storage.
     */
    private updateResourcesAuthInfo;
    /**
     * read resource auth info from session storage.
     */
    private readResourcesAuthInfo;
    ...
    private getOrgUrlFromFederatedService;
    /**
    ...
    getClientIdByPortalUrlAndHost(portalUrl: string, hostName: string): string;
     ...
     * This method will try to sign in to get a new session if the 'gotoSignInDirectly' property is true.
     * Otherwise, it will just handle the token error by different scenarios:
     *   498: token invalided, go to system error page;
     *   token expired: show runtime error dialog to remind user to re-sign in;
     *   app item does not exist (item is deleted) or is inaccessible: show runtime error reminder.
     */
    handleAuthError: (handleAuthErrorParams: HandleAuthErrorParams) => Promise<ArcGISIdentityManager>;
    /**
     ...
     * @param owningSystemUrl owning system url for federated service url
     */
    signOutByResourceUrl(resourceUrl: string, owningSystemUrl?: string): void;
    /**
     * Start OAuth2 sign-out flow
     * @param signOutParams The properties for sign-out.
     */
    signOut(signOutParams?: SignOutParams): void;
    /**
     ...
     * @param owningSystemUrl owning system url for federated service url
     */
    switchAccountByResourceUrl(resourceUrl: string, owningSystemUrl?: string): Promise<ArcGISIdentityManager>;
    /**
     * Start OAuth2 switch account flow and return the user session.
     *
     * @param switchAccountParams The properties for switching account.
     */
    switchAccount(switchAccountParams?: SwitchAccountParams): Promise<ArcGISIdentityManager>;
    private defer;
    /**
     * Generate token.
     * @param portalUrl
     ...
     * @param getTokenParam
     */
    checkWebTierPortalAndGetSession(portalUrl: string, getTokenParam?: string, portalSelfParam?: any): Promise<ArcGISIdentityManager>;
    /**
     * @ignore
     * check the server is a web-tier server and get session by server url. (webTier services are stored in the 'trusted servers')
     * @param serviceUrl
     */
    checkWebTierServerAndGetSession(serviceUrl: string): Promise<ArcGISIdentityManager>;
    /**
    ...
    /**
     * Sync web-tier resources to other window's session manager
     */
    private syncWebTierResourcesToOtherWindow;
    /**
     * Sync web-tier resources from other window, this is for build-app sync only.
     *
     * @ignore
     */
    syncWebTierResourcesFromOtherWindow(webTierResources: string[]): void;
    ...
    /**
     * Get resource session key for url
     *
     * @param url service or portal (item) url
     * @param owningSystemUrl it's owning system url of federated service
     *
     * @ignore
     */
    getResourcesSessionKey(url: string, owningSystemUrl?: string): string;
}
/** @ignore */
