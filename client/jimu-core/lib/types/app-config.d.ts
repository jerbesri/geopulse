...
import type { AdvancedExpression } from './expression';
...
export type A11yLandmarks = 'banner' | 'main' | 'navigation' | 'contentinfo' | 'region' | 'complementary' | 'search';
export interface WidgetJson {
    ...
    */
    ...
    /**
     * This property decides how the widget is rendered in a widget controller.
     * A widget can update this property in it's setting so it can display well in the controller widget.
     *
     * - 'inPanel': The default value. The widget will be rendered in the controller's panel.
     * - `offPanel`: The widget will be rendered as it is besides the controller without a panel.
     * - `inController`: The widget will be rendered as it is inside the controller button list.
     *
     * Note:
     * - This property is valid only when a widget is placed in a widget controller.
     * - For `inController`, a widget can use jimu-ui `WidgetButton` component so the widget can fit well in the controller.
     *
     * @default 'inPanel'
     */
    inControllerUx?: 'inPanel' | 'offPanel' | 'inController';
    /**
     * Accessible label for the widget, used by screen readers.
     * When a value is set, it will be applied as an `aria-label` on the `widget-content` DOM element (with role='group') by framework.
     * If widget needs to handle the `a11yLabel` itself (such as iframe case), it can set `handleA11yLabelInWidget` to `true` in manifest's properties, then handle it within the widget.
     * @default ''
     */
    a11yLabel?: string;
    /**
     * ARIA landmark role for the widget, used by screen readers to identify the type of content.
     * @default ''
     */
    a11yLandmark?: A11yLandmarks;
    /**
     * Indicates whether the widget is added to `Skip To Content`, including root widget from body, header(enabled by default), and footer(enabled by default). If a widget is not at the root level, it's not included even if it has this property.
     * When set to `true`, the framework will add `tabIndex=-1`,`id`, `aria-label`, and `role=group` on the `widget-content` DOM element.
     * If widget needs to handle the `addToA11ySkip` itself (such as iframe case), it can set `handleA11yLabelInWidget` to `true` in manifest's properties, then add the `id` within the widget.
     * @default false
     */
    addToA11ySkip?: boolean;
}
...
export type DateTimeFieldFormatProperties = __esri.DateTimeFieldFormatProperties & {
    type: 'date-time';
};
export type NumberFieldFormatProperties = __esri.NumberFieldFormatProperties & {
    type: 'number';
};
export type FieldFormatProperties = DateTimeFieldFormatProperties | NumberFieldFormatProperties;
/**
 * The esri field format do not include the hour and minute properties.
 * This interface was created to extend these attributes, allowing for customization of the display formats from intl.
 */
export interface DateWithTimeFieldFormatProperties extends DateTimeFieldFormatProperties, Pick<Intl.DateTimeFormatOptions, 'day' | 'hour' | 'minute' | 'second'> {
}
export declare enum FieldSourceType {
    /**
     * 'NORMAL' indicates that this field comes from the service, which is the most common case. The default value is 'NORMAL'.
     */
    Normal = "NORMAL",
    /**
     * 'CUSTOM_ARCADE_FIELD' indicates that the field comes from a custom-added Arcade field.
     */
    CustomArcadeField = "CUSTOM_ARCADE_FIELD"
}
export interface FieldSchema {
     ...
     * 1. https://developers.arcgis.com/javascript/latest/api-reference/esri-layers-support-FieldConfiguration.html
     * 2. https://developers.arcgis.com/web-map-specification/objects/fieldConfiguration/
     */
    fieldFormat?: FieldFormatProperties;
    /**
     * @deprecated
     * Use fieldFormat instead.
     */
    ...
    /**
     * @ignore
     * This attribute is only used during development and is not stored in the config.
     */
...
export interface DataSourceSchema {
    ...
    /**
     * Custom Arcade fields added by users.
     */
    arcadeFields?: {
        [arcadeFieldName: string]: AdvancedExpression;
    };
}
/**
...
export interface AttributesJson {
    ...
    */
    express?: boolean;
    /**
     * Whether show accessibility settings for each widget.
     * @default false
    */
...
export type IMSavedTheme = ImmutableObject<SavedTheme>;
...
export interface Permission {
    ...
    /***
    If this property exists, it means the page access control is enabled, only the allowed pages are accessible.
    Implement details:
    * In builder, we don't check the permission, this means users can edit the app can always see all the content of the app. Owner and admin can edit the permission, and the users in a shared-update-group can edit app but can not edit the permission.
    * When app is launched, we'll check the permission. The pages (and their content) that the user doesn't have permission will be removed from the app config.
    **/
...
export type IMTranslation = ImmutableObject<Translation>;
export interface AppResource {
    /**
     * This property is the identifier of the resource in the app resource management.
      * The URL is in the format: '${appResourceUrl}/widgets/widget_1/1641535897496.png'.
     *
     * Using `appConfigUtils.processResourceUrl`, it can be converted to the real URL format, such as:
     * 'https://www.arcgis.com/sharing/rest/content/items/787381a21f7/resources/widgets/widget_1/1641535897496.png?token=...'
     *
     * Note:
     * - For `Image`, `ImageViewer` and `Icon` components, the URL is already converted, so no additional conversion is needed.
     * - For `<img>` tags, CSS `url()` properties or other usages, you must manually convert the URL using `appConfigUtils.processResourceUrl`.
     */
    url: string;
    originalName?: string;
    fileFormat?: string;
...
export type IMAppResource = ImmutableObject<AppResource>;
export interface WidgetResourceList {
    [widgetId: string]: {
        [resource: string]: AppResource;
    };
}
export type IMWidgetResourceList = ImmutableObject<WidgetResourceList>;
export interface AppResourceList {
    widgetResourceList?: WidgetResourceList;
}
export type IMAppResourceList = ImmutableObject<AppResourceList>;
/**
...
export interface AppConfig {
    ...
    */
    ...
    /** The locale when the app is created. */
    mainLocale?: string;
    /**
     * Translations for the multi-lang setting panel.
    ...
    /** The translation values that are changed by user manually. */
    changedTranslationValues?: string[];
    /**
     * The resources used in the app.
     * This property is loaded when app is loaded and won't be saved in the app config file.
     */
    appResourceList?: AppResourceList;
}
