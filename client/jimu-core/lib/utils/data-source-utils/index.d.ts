import type { IField, ILayerDefinition, IPopupInfo } from '@esri/arcgis-rest-feature-service';
import type { IMExportOptions, IMFieldSchema, IMUseDataSource, UseDataSource } from '../../types/app-config';
...
/**
 * The AGOL item includes three types:
 * - hosted service
 * - proxy service (AGOL/enterprise)
 * - helper services (ignored)
 */
export declare function isAGOLHostedItemOrService(dataSource: QueriableDataSource): Promise<boolean>;
/**
 * The enterprise service includes relational datastore and spatiotemporal datastore.
 * For relational datastore, it supports char_length, lower, upper, lower+in operators.
 * For spatiotemporal datastore, it supports them with limitations.
 *   - lower and upper are supported
 *   - char_length is not supported
 *   - lower+in is supported since 12.0
 */
export declare function isRelationalDataStore(dataSource: QueriableDataSource): boolean;
export declare function isCharLengthSupported(dataSource: QueriableDataSource): boolean;
export declare function isLowerInSupported(dataSource: QueriableDataSource): boolean;
...
export declare function getJimuUseDataSourcesRelationship(messageUseDs: ImmutableObject<UseDataSource> | UseDataSource, actionUseDs: ImmutableObject<UseDataSource> | UseDataSource): JimuDataSourceRelationship;
export declare const NON_COUNT_AFFECTING_QUERY_PARAMS_KEYS: Array<keyof ArcGISQueryParams>;
export declare const NON_FEATURE_AFFECTING_QUERY_PARAMS_KEYS: Array<keyof ArcGISQueryParams>;
...
export * from './clear-data-source-filter-utils';
