...
export type ArcGISQueriableLayerTypes = __esri.FeatureLayer | __esri.SceneLayer | __esri.OrientedImageryLayer | __esri.ImageryLayer | ArcGISSubtypeGroupLayer | ArcGISSubtypeSublayer | __esri.KnowledgeGraphSublayer
...
export interface ArcGISQueriableDataSource extends QueriableDataSource, JSAPILayerMixin, ItemMixin {
  ...
  /** Return the current historic moment. */
  getHistoricMoment: () => string | number
  /** Change the current historic moment. */
  changeHistoricMoment: (moment: string | number) => void
  /**
  ...
  selectRecordById: (id: string | number, record?: FeatureDataRecord) => void
  selectRecordsByIds: (ids: Array<string | number>, records?: FeatureDataRecord[]) => void
  ...
  /**
   * When current data source is created from an array of records (`FeatureDataRecord`), will save it in `sourceRecords`.
...
export interface FeatureDataRecord extends DataRecord {
  ...
  resolveArcadeFieldValue: (arcadeFieldName: string) => Promise<string | number | Date | null>
}
