import {
  ...
  type MapWidgetInfo, type RuntimeErrors, type RuntimeInfos, type UrlParameters, type WidgetsMutableStateVersion,
  type IMAppConfig,
  version
} from 'jimu-core'
...
export function initGlobal () {
  window.jimuConfig = {
    ...
    hostEnv: 'prod',
    isDevEdition: false,
    arcgisJsApiUrl: 'https://js.arcgis.com/5.1/',
...
export function getInitState (): IMState {
  ...
  const INIT_STATE: IMState = Immutable({
    ...
    systemError: null,
    runtimeErrors: {} as RuntimeErrors,
